...
async def main():
    ...
    dp.message.outer_middleware(LocalizationMiddleware(redis=config.REDIS))
    dp.callback_query.outer_middleware(LocalizationMiddleware(redis=config.REDIS))
    dp.pre_checkout_query.outer_middleware(LocalizationMiddleware(redis=config.REDIS))
