...
async def get_default_close_button(user_lang) -> InlineKeyboardButton:
    return InlineKeyboardButton(
        text=LOC[user_lang]["buttons"]["close"], callback_data="default_close_menu"
    )
            ...
            try:
                ...
                if "message to delete not found" in err.message:
                    ...
                    config.LOGGER.warning(
                        f"Unknown error on message delete: {err.message}"
                    )
    except KeyError:
        config.LOGGER.debug("expected KeyError, No messages to delete")
    await state.clear()


async def user_space_remaining_percent(tg_uid) -> str:
    space_taken = await db_req.get_user_attr(
    ...
    return await _format_user_space_remaining_percent(
        space_taken=space_taken, subscribed=subscribed
    )


async def _format_user_space_remaining_percent(
    space_taken: int, subscribed: bool
) -> str:
    if subscribed:
...
async def user_space_remaining_mb(tg_uid) -> str:
    ...
    return await _format_user_space_remaining_mb(
        subscribed=subscribed, space_taken=space_taken
    )
...
async def _format_user_space_remaining_mb(space_taken: int, subscribed: bool) -> str:
    ...
    if remaining_mb >= 0.1:
        round_method = "ROUND_DOWN"
    return (
        f"{Decimal(remaining_mb).quantize(Decimal('0.001'), round_method)}<b>|</b>0.1MB"
    )
