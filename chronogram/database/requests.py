...
from .models import (
    InnerChronogramUserData,
    InnerChronogramPaymentData,
    InnerTimeCapsuleData,
    OuterChronogramPaymentData,
    OuterTimeCapsuleData,
)
...
async def get_uid_by_tg_uid(tg_uid: int) -> int:
    async with async_session() as session:
        return await session.scalar(
            select(ChronogramUser.id).where(ChronogramUser.tg_uid == tg_uid)
        )
    ...
    if not await get_uid_by_tg_uid(tg_uid):
        async with async_session() as session:
            ...
            if lang not in ("en", "ru"):
                lang = "en"
            data = InnerChronogramUserData(
                language=lang, utc_offset_minutes=utc_offset, tg_uid=tg_uid
            )
            await session.execute(
                insert(ChronogramUser).values(
                    language=data.language,
                    utc_offset_minutes=data.utc_offset_minutes,
                    tg_uid=tg_uid,
                    joined=datetime.datetime.utcnow().replace(microsecond=0),
                )
            )
...
class TimeCapsuleDatabaseActions:
        ...
        async with async_session() as session:
            data = [
                ...
                for x in (
                    await session.execute(
                        select(TimeCapsule)
                        .where(TimeCapsule.received == False)
                        .where(
                            TimeCapsule.receive_timestamp <= datetime.datetime.utcnow()
                        )
                    )
                ).all()
            ]
    ...
    async def timecapsules_received(tg_uid) -> bool:
        async with async_session() as session:
            return bool(
                await session.scalar(
                    select(TimeCapsule.id)
                    .where(TimeCapsule.received == True)
                    .where(TimeCapsule.user_id == await get_uid_by_tg_uid(tg_uid))
                )
            )
    ...
    async def get_timecapsules_for_deletion(tg_uid: int) -> list[TimeCapsule]:
        ...
        async with async_session() as session:
            return (
                await session.execute(
                    select(TimeCapsule.id, TimeCapsule.size)
                    .where(TimeCapsule.user_id == await get_uid_by_tg_uid(tg_uid))
                    .order_by(
                        desc(TimeCapsule.received), desc(TimeCapsule.send_timestamp)
                    )
                )
            ).all()
    ...
    async def create_timecapsule(data: OuterTimeCapsuleData):
        async with async_session() as session:
            inner = InnerTimeCapsuleData(
                user_id=await get_uid_by_tg_uid(data.tg_uid),
                send_timestamp=data.send_timestamp,
                receive_timestamp=data.receive_timestamp
                - datetime.timedelta(
                    minutes=await get_user_attr(
                        tg_uid=data.tg_uid, col=ChronogramUser.utc_offset_minutes
                    )
                ),
                text_content=(
                    fernet.encrypt(data.text_content.encode("utf-8"))
                    if data.text_content
                    else None
                ),
                size=data.size,
                image=fernet.encrypt(data.image) if data.image else None,
                image_data=data.image_data,
            )
            await session.execute(
                insert(TimeCapsule).values(
            ...
            await session.execute(
                update(ChronogramUser)
                .where(ChronogramUser.id == await get_uid_by_tg_uid(data.tg_uid))
                .values(
                    space_available=await get_user_attr(
                        tg_uid=data.tg_uid, col=ChronogramUser.space_available
                    )
                    - data.size,
                    space_taken=await get_user_attr(
                        tg_uid=data.tg_uid, col=ChronogramUser.space_taken
                    )
                    + data.size,
                )
            )
    ...
    async def delete_timecapsule(tg_uid: int, tc_id: int):
        async with async_session() as session:
            size_to_free = await session.scalar(
                select(TimeCapsule.size)
                .where(TimeCapsule.id == tc_id)
                .where(TimeCapsule.user_id == await get_uid_by_tg_uid(tg_uid))
            )
            ...
            await session.execute(
                update(ChronogramUser)
                .where(ChronogramUser.id == await get_uid_by_tg_uid(tg_uid))
                .values(
                    space_available=await get_user_attr(
                        tg_uid=tg_uid, col=ChronogramUser.space_available
                    )
                    + size_to_free,
                    space_taken=await get_user_attr(
                        tg_uid=tg_uid, col=ChronogramUser.space_taken
                    )
                    - size_to_free,
                )
            )
    ...
    async def delete_everything(tg_uid: int):
        async with async_session() as session:
            ...
            await session.execute(
                ...
                .values(
                    space_available=(
                        config.DEFAULT_USER_SPACE
                        if not await get_user_attr(
                            user_id=uid, col=ChronogramUser.subscription
                        )
    ...
    @staticmethod
    async def is_enough_space(
        tg_uid: int, text: str, image=Optional[bytearray]
    ) -> bool:
        content_size = await TimeCapsuleDatabaseActions.get_timecapsule_size(
            text, image
        )
        available_space = await get_user_attr(
            tg_uid=tg_uid, col=ChronogramUser.space_available
        )
...
async def get_subscribers_with_expiring_subscription() -> list[ChronogramUser]:
    async with async_session() as session:
        return [
            ...
            for x in (
                await session.execute(
                    select(ChronogramUser)
                    .where(ChronogramUser.subscription_deadline is not None)
                    .where(ChronogramUser.notified_deadline == False)
                    .where(
                        datetime.datetime.utcnow()
                        >= (
                            ChronogramUser.subscription_deadline
...
async def mark_as_notified(tg_uid: int):
    async with async_session() as session:
        await session.execute(
            update(ChronogramUser)
            .where(ChronogramUser.id == await get_uid_by_tg_uid(tg_uid=tg_uid))
...
async def edit_utc_diff(tg_uid, value: int):
    async with async_session() as session:
        await session.execute(
            update(ChronogramUser)
            .where(ChronogramUser.id == await get_uid_by_tg_uid(tg_uid))
            .values(utc_offset_minutes=value)
        )
        await session.commit()
...
async def revoke_subscription(tg_uid: int):
    ...
    if space_taken > config.DEFAULT_USER_SPACE:
        for tc in await TimeCapsuleDatabaseActions.get_timecapsules_for_deletion(
            tg_uid
        ):
            await TimeCapsuleDatabaseActions.delete_timecapsule(
                tg_uid=tg_uid, tc_id=tc.id
            )
            space_taken -= tc.size
            if space_taken <= config.DEFAULT_USER_SPACE:
                break
    async with async_session() as session:
        await session.execute(
            update(ChronogramUser)
            .where(ChronogramUser.id == user_id)
            .values(
...
async def invoice_exists(invoice_id: str):
    async with async_session() as session:
        if await session.scalar(
            select(ChronogramPayment.invoice_id).where(
                ChronogramPayment.invoice_id == invoice_id
            )
        ):
            return True
        return False
...
async def process_refund(tg_uid: int, telegram_payment_charge_id: str):
    async with async_session() as session:
        pay_data: list[ChronogramPayment] = (
            await session.execute(
                select(ChronogramPayment)
                .where(
                    ChronogramPayment.tg_transaction_id == telegram_payment_charge_id
                )
    ...
    async with async_session() as session:
        await session.execute(
            ...
            .values(
                ...
                space_available=config.PREMIUM_USER_SPACE
                - await get_user_attr(user_id=user_id, col=ChronogramUser.space_taken),
            )
        )
        await session.commit()
...
async def _prolong_subscription(user_id: int, months: int):
    async with async_session() as session:
        cur_deadline = await get_user_attr(
            user_id=user_id, col=ChronogramUser.subscription_deadline
        )
        await session.execute(
            update(ChronogramUser)
            .where(ChronogramUser.id == user_id)
            .values(subscription_deadline=cur_deadline + relativedelta(months=months))
        )
        await session.commit()


async def process_payment(
    pay_data: OuterChronogramPaymentData, months: int = None
) -> str:
    response = "DONATE"
    async with async_session() as session:
        user_id = await get_uid_by_tg_uid(pay_data.tg_uid)
        inner_data = InnerChronogramPaymentData(
            user_id=user_id,
        ...
        await session.execute(
            insert(ChronogramPayment).values(
                timestamp=inner_data.timestamp,
                user_id=user_id,
        ...
        if pay_data.type == "subscription":
            if await session.scalar(
                select(ChronogramUser.subscription).where(ChronogramUser.id == user_id)
            ):
                print("prolong")
                response = "PROLONG"
...
async def get_stats() -> tuple[int, int, int]:
    async with async_session() as session:
        ...
        subs_bought = (
            await session.execute(
                select(func.count(ChronogramPayment.id)).where(
                    ChronogramPayment.type == "subscription"
                )
            )
        ).scalar_one()
