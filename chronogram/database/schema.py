...
class ChronogramUser(Base):
    ...
    subscription_deadline: Mapped[TIMESTAMP] = mapped_column(
...
class TimeCapsule(Base):
    ...
    image_data: Mapped[VARCHAR] = mapped_column(
        VARCHAR(20), default=None, nullable=True
    )
    ...
    def __repr__(self):
        values = ", ".join(
            (
                f"{key}={bool(self.__dict__[key])}"
                if key in ("text_content", "image")
                else f"{key}={self.__dict__[key]}"
            )
