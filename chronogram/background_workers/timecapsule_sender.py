    ...
    while True:
        ...
        if timecapsules:
            for tc in timecapsules:
                tg_uid = await db_req.get_user_attr(
                    user_id=tc.user_id, col=ChronogramUser.tg_uid
                )
                user_lang = await db_req.get_user_attr(
                    user_id=tc.user_id, col=ChronogramUser.language
                )
                await send_timecapsule(
                    tg_uid=tg_uid,
                    sent=tc.send_timestamp,
                    tc_id=tc.id,
                    photo=tc.image,
                    content=tc.text_content,
                    l10n=await get_l10n_by_lang(lang=user_lang),
                )
                await TC.mark_as_received(tg_uid=tg_uid, tc_id=tc.id)
                config.LOGGER.info(
                    f"Sent timecapsule to ...{str(tg_uid)[5:]}, "
                    f"abs time = {tc.receive_timestamp - tc.send_timestamp}"
                )
        await asyncio.sleep(5)


async def get_formatted_msg_content(
    tg_uid: int, content: bytes, sent: datetime.datetime, l10n: L10N
) -> str:
    ...
    if content:
        formatted_msg = l10n.data["/timecapsule"]["received"].format(
            datetime.datetime.strftime(
                sent + datetime.timedelta(minutes=utc_diff), "%H:%M %d.%m.%Y"
            ),
            config.FERNET.decrypt(content).decode("utf-8"),
        )
    else:
        formatted_msg = l10n.data["/timecapsule"]["received"].format(
            datetime.datetime.strftime(
...
async def process_selection(
    callback: CallbackQuery, data: KeepOrDeleteCallback, l10n: L10N
):
    ...
    match data.action:
        case KeepOrDeleteActions.keep_message:
            await callback.answer(
                text=l10n.data["/timecapsule"]["saved"].format(
                    await user_space_remaining_percent(data.tg_uid)
                )
            )
            try:
                ...
                if photo:
                    await callback.message.edit_caption(
                        caption=l10n.data["/timecapsule"]["saved"]
                    )
                else:
                    await callback.message.edit_text(
                        text=l10n.data["/timecapsule"]["saved"]
                    )
        case KeepOrDeleteActions.delete_message:
                ...
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            ...
                            callback_data=KeepOrDeleteCallback(
                                action=KeepOrDeleteActions.confirm_delete,
                                tg_uid=data.tg_uid,
                                tc_id=data.tc_id,
                            ).pack(),
                        ),
            ...
            if photo:
                await callback.message.edit_caption(
                    caption=await get_formatted_msg_content(
                        data.tg_uid, tc.text_content, tc.send_timestamp, l10n=l10n
                    )
                ...
                return
            await callback.message.edit_text(
                text=await get_formatted_msg_content(
                    data.tg_uid, tc.text_content, tc.send_timestamp, l10n=l10n
                ),
                reply_markup=markup,
            )
        ...
        case KeepOrDeleteActions.confirm_delete:
            ...
            try:
                ...
                if photo:
                    await callback.message.edit_caption(
                        caption=l10n.data["/timecapsule"]["deleted"]
                    )
                else:
                    await callback.message.edit_text(
                        text=l10n.data["/timecapsule"]["deleted"]
                    )
