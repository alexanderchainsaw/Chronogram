...
class TimepickerCallback(CallbackData, prefix="timepicker"):
    ...
    def replace_and_pack(self, action: TimepickerActions) -> str:
        return TimepickerCallback(
            action=action, user_id=self.user_id, hour=self.hour, minute=self.minute
        ).pack()
...
async def start_timepicker(
    ...
    keyboard[0] = [
        InlineKeyboardButton(
            text="↑",
            callback_data=data.replace_and_pack(action=TimepickerActions.INCR_HOUR_1),
        ),
        InlineKeyboardButton(
            text="⇈",
            callback_data=data.replace_and_pack(action=TimepickerActions.INCR_HOUR_4),
        ),
        InlineKeyboardButton(
            text="⇈",
            callback_data=data.replace_and_pack(action=TimepickerActions.INCR_MIN_15),
        ),
        InlineKeyboardButton(
            text="↑",
            callback_data=data.replace_and_pack(action=TimepickerActions.INCR_MIN_1),
        ),
    ]
    ...
    keyboard[2] = [
        InlineKeyboardButton(
            text="↓",
            callback_data=data.replace_and_pack(action=TimepickerActions.DECR_HOUR_1),
        ),
        InlineKeyboardButton(
            text="⇊",
            callback_data=data.replace_and_pack(action=TimepickerActions.DECR_HOUR_4),
        ),
        InlineKeyboardButton(
            text="⇊",
            callback_data=data.replace_and_pack(action=TimepickerActions.DECR_MIN_15),
        ),
        InlineKeyboardButton(
            text="↓",
            callback_data=data.replace_and_pack(action=TimepickerActions.DECR_MIN_1),
        ),
    ]
...
def process_time_change(
    hh_mm: tuple[int, int], to_add: tuple[int, int], sign: str
) -> tuple[int, int]:
    if sign == "+":
...
async def process_selection(
    ...
    match data.action:
        ...
        case TimepickerActions.INCR_MIN_1:
            data.hour, data.minute = process_time_change(
                (data.hour, data.minute), (0, 1), "+"
            )
            await callback.message.edit_reply_markup(
                reply_markup=await start_timepicker(data, l10n=l10n)
            )
        case TimepickerActions.INCR_MIN_15:
            data.hour, data.minute = process_time_change(
                (data.hour, data.minute), (0, 15), "+"
            )
            await callback.message.edit_reply_markup(
                reply_markup=await start_timepicker(data, l10n=l10n)
            )
