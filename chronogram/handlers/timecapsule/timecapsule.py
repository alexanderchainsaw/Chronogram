...
from ...utils import (
    user_space_remaining_percent,
    perform_state_clear,
    get_default_close_button,
)
...
from .helpers import (
    _get_not_enough_space_message,
    _validate_timecapsule,
    parse_date_from_message,
    parse_content_from_message,
...
async def create_timecapsule(message: Message, state: FSMContext, l10n: L10N) -> None:
    ...
    prompt_content_msg_id = (
        await message.answer(
            text=l10n.data["/timecapsule"]["init"],
...
async def timecapsule_prompt_date(message: Message, state: FSMContext, l10n: L10N):
    ...
    if not valid:
        ...
        match tc_type:
            case "Space":
                await message.answer(
                    text=await _get_not_enough_space_message(
            ...
            case "Data":
                await message.answer(
                    text=l10n.data["/timecapsule"]["invalid_data"],
                    reply_markup=keyboard,
                )
            case "Length":
                if message.photo:
                    await message.answer(
                        text=l10n.data["/timecapsule"]["invalid_caption_length"].format(
                            len(message.text)
                        ),
                        reply_markup=keyboard,
                    )
                    ...
                    await message.answer(
                        text=l10n.data["/timecapsule"]["invalid_length"].format(
                            len(message.text)
                        ),
                        reply_markup=keyboard,
                    )
        try:
            ...
            await config.BOT.delete_message(
                message_id=data["prompt_content_msg_id"], chat_id=message.from_user.id
            )
    ...
    user_time = datetime.utcnow() + timedelta(
    ...
    calendar.set_dates_range(
        datetime(user_time.year, user_time.month, user_time.day),
        datetime(user_time.year + 1000, user_time.month, user_time.day),
    )

    message_response = (
        "<blockquote>{}</blockquote>\n\n".format(
            html.escape(timecapsule_text) if timecapsule_text else ""
        )
        + f"{l10n.data['/timecapsule']['prompt_date']}"
    )
    if message.photo:
        ph = PhotoReader(
            photo=message.photo, file_name=f"temp\\create_{message.from_user.id}.jpg"
        )
        ...
        await message.answer_photo(
            ...
            reply_markup=await calendar.start_calendar(
                l10n=l10n, year=user_time.year, month=user_time.month
            ),
        )
        await ph.delete()
...
async def process_date_selection(
    callback: CallbackQuery, callback_data: SimpleCalendarCallback, l10n: L10N
):
    calendar = SimpleCalendar(locale=l10n.lang, show_alerts=True)
    user_time = datetime.utcnow() + timedelta(
        minutes=await db_req.get_user_attr(
    ...
    selected, date, canceled = await calendar.process_selection(
        callback, callback_data, l10n=l10n
    )

    if selected:
        content = await parse_content_from_message(callback)
        timepicker_callback = TimepickerCallback(
            user_id=callback.from_user.id, hour=user_time.hour, minute=user_time.minute
        )
        message_response = (
        ...
        if callback.message.photo:
            await callback.message.edit_caption(
                caption=message_response,
                reply_markup=await start_timepicker(
                    data=timepicker_callback, l10n=l10n
                ),
            )
            return
...
async def process_time(
    callback: CallbackQuery, callback_data: TimepickerCallback, l10n: L10N
):
    selected, _time, canceled = await process_selection(
        callback, callback_data, l10n=l10n
    )
    if selected:
        selected_date = datetime.strptime(
            await parse_date_from_message(callback), "%d.%m.%Y"
        )
...
async def confirm_send(callback: CallbackQuery, _time, l10n: L10N):
    ...
    msg += l10n.data["/timecapsule"]["confirm_no_text"].format(
        f"{date} {_time.strftime('%H:%M')}"
    )
    if callback.message.photo:
        await callback.message.edit_caption(
            caption=msg,
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text=l10n.data["buttons"]["cancel"], callback_data="send_cancel"
                    ),
                    InlineKeyboardButton(
                        text=l10n.data["buttons"]["send"], callback_data="send_confirm"
                    ),
                ]
            ]
        ),
...
async def message_sent(callback: CallbackQuery, l10n: L10N):
    ...
    if callback.message.photo:
        ph = PhotoReader(
            ...
            file_name=f"temp\\create_{callback.from_user.id}.jpg",
        )
    ...
    await TC.create_timecapsule(
        data=OuterTimeCapsuleData(
            tg_uid=callback.from_user.id,
            text_content=content,
            receive_timestamp=datetime.strptime(
                date + " " + _time, "%d.%m.%Y %H:%M"
    ...
    await callback.answer(
        l10n.data["/timecapsule"]["sent"].format(
            await user_space_remaining_percent(tg_uid=callback.from_user.id)
        )
    )
