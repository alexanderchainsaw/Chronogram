...
class InboxCallback(CallbackData, prefix="inbox"):
    ...
    async def replace_and_pack(
        self, action: InboxCallbackActions, cur_timecapsule=None
    ) -> str:
        return InboxCallback(
            ...
            cur_timecapsule=cur_timecapsule,
        ).pack()
...
async def get_message_timestamps_fmt(tc_id, tg_uid) -> str:
    ...
    receive_t = (tc_data.receive_timestamp + timedelta(minutes=utc_diff)).strftime(
        "%H:%M %d.%m.%Y"
    )
    send_t = (tc_data.send_timestamp + timedelta(minutes=utc_diff)).strftime(
        "%H:%M %d.%m.%Y"
    )
...
async def get_message_content_fmt(tc_id, tg_uid, l10n: L10N) -> str:
    ...
    if tc_data.text_content:
        return l10n.data["/inbox"]["timecapsule_data"].format(
            send_t,
            receive_t,
            config.FERNET.decrypt(tc_data.text_content).decode("utf-8"),
        )
...
async def start_inbox_caption(tg_uid, l10n: L10N, empty: bool = False) -> str:
    if empty:
        return l10n.data["/inbox"]["empty"] + l10n.data["/inbox"][
            "some_underway"
        ] * await TC.timecapsules_underway(tg_uid)
    subscribed = await db_req.get_user_attr(
        tg_uid=tg_uid, col=ChronogramUser.subscription
    )
    msg_text = l10n.data["/inbox"]["init"].format(
        await user_space_remaining_mb(tg_uid),
...
async def start_inbox_menu(data: InboxCallback, l10n: L10N) -> InlineKeyboardMarkup:
    ...
    builder.add(
        InlineKeyboardButton(
            text=l10n.data["/inbox"]["menu_col_sent"],
            callback_data=await data.replace_and_pack(
                action=InboxCallbackActions.ignore
            ),
        ),
        InlineKeyboardButton(
            text=f"{data.cur_page + 1}/{data.total_pages}",
            callback_data=await data.replace_and_pack(
                action=InboxCallbackActions.ignore
            ),
        ),
        InlineKeyboardButton(
            text=l10n.data["/inbox"]["menu_col_received"],
            callback_data=await data.replace_and_pack(
                action=InboxCallbackActions.ignore
            ),
        ),
    ...
    for msg in packed_timecapsules[data.cur_page]:
        builder.button(
            text=await get_message_timestamps_fmt(msg, data.user_id),
            callback_data=await data.replace_and_pack(
...
async def process_selection(callback: CallbackQuery, data: InboxCallback, l10n: L10N):
        ...
        match data.action:
            ...
            case InboxCallbackActions.back_to_page:
                ...
                await callback.message.edit_media(
                    media=InputMediaPhoto(media=inbox_pic)
                )
                await callback.message.edit_caption(
                    caption=await start_inbox_caption(data.user_id, l10n=l10n),
                    parse_mode="HTML",
                )
                await callback.message.edit_reply_markup(
                    reply_markup=await start_inbox_menu(data, l10n=l10n)
            ...
            case InboxCallbackActions.delete_timecapsule:
                await callback.message.edit_caption(
                    caption=l10n.data["/timecapsule"]["delete"],
    ...
    if data.action.startswith("GET_TIMECAPSULE"):
        try:
            ...
            photo_data = await TC.get_timecapsule_image_data(
                tg_uid=data.user_id, tc_id=tc_id
            )
            if photo:
                img = Image.frombytes(
                    data=config.FERNET.decrypt(photo),
            ...
            await callback.message.edit_caption(
                caption=await get_message_content_fmt(tc_id, data.user_id, l10n=l10n),
                parse_mode="HTML",
            )
            await callback.message.edit_reply_markup(
                reply_markup=InlineKeyboardMarkup(
...
async def handle_delete_from_inbox(data: InboxCallback, callback, l10n: L10N):
    ...
    if not received_messages:
        await callback.answer(
            text=await start_inbox_caption(data.user_id, empty=True, l10n=l10n)
        )
        await callback.message.delete()
        return
    await callback.answer(
        l10n.data["/timecapsule"]["deleted"].format(
            await user_space_remaining_percent(data.user_id)
        )
    )
