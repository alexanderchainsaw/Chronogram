...
async def cmd_delete_everything(message: Message, state: FSMContext, l10n: L10N):
    ...
    cancel = InlineKeyboardButton(
        text=l10n.data["buttons"]["cancel"], callback_data="cancel_delete_everything"
    )
    prompt_message_id = (
        await message.answer(
            text=l10n.data["/delete_everything"]["init"],
...
async def confirm_delete_everything(message: Message, state: FSMContext, l10n: L10N):
    ...
    if message.text != l10n.data["/delete_everything"]["confirmation_message"]:
        await message.answer(
            text=l10n.data["/delete_everything"]["confirmation_message_invalid"],
            reply_markup=keyboard,
        )
    else:
        await db_req.TimeCapsuleDatabaseActions.delete_everything(
            tg_uid=message.from_user.id
        )
        await message.answer(
            text=l10n.data["/delete_everything"]["success"], reply_markup=keyboard
        )
    try:
        data = await state.get_data()
        await config.BOT.delete_message(
            chat_id=message.from_user.id, message_id=data["prompt_message_id"]
        )
    except (aiogram.exceptions.TelegramBadRequest, KeyError) as e:
        config.LOGGER.warning(
            f"Error on delete_everything (delete prompt message), error: {e}"
        )
    await state.clear()
    await message.delete()
