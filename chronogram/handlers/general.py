...
async def command_start(message: Message, state: FSMContext, l10n: L10N):
    ...
    utc_offset = await db_req.get_user_attr(tg_uid=message.from_user.id, col=ChronogramUser.utc_offset_minutes)
    await message.answer(text=l10n.data['/start'].format({'ru': '🇷🇺', 'en': '🇬🇧'}[l10n.lang],
                                                         (datetime.utcnow() + timedelta(minutes=utc_offset))
                                                         .strftime('%H:%M %d.%m.%Y')))
