...
from .schemas import (
    InvoicePayloadData,
    process_subscription_menu_actions,
    SubscriptionMenuCallback,
)
...
async def cmd_donate(message: Message, state: FSMContext, l10n: L10N):
    ...
    if len(data) != 2:
        ...
        await message.answer_invoice(
            title=l10n.data["/donate"]["confirm_title"],
...
async def process_successful_payment(message: Message, l10n: L10N):
    ...
    if await db_req.get_user_attr(
        col=ChronogramUser.subscription, tg_uid=message.from_user.id
    ):
        cur_deadline = await db_req.get_user_attr(
            col=ChronogramUser.subscription_deadline, tg_uid=message.from_user.id
        )
        active_until = (
            (
                cur_deadline
    ...
    match r:
        case "DONATE":
            await message.answer(
                text=l10n.data["/donate"]["success"],
                message_effect_id="5104841245755180586",
                reply_markup=keyboard,
            )
        case "PROLONG":
            await message.answer(
                text=l10n.data["/subscription"]["subscription_prolonged"].format(
        ...
        case "GRANT":
            await message.answer(
                text=l10n.data["/subscription"]["subscription_activated"].format(
                    datetime.strftime(active_until, "%H:%M %d.%m.%Y")
                ),
                reply_markup=keyboard,
            )
...
async def cmd_refund(message: Message, state: FSMContext, l10n: L10N):
    ...
    user_lang = await db_req.get_user_attr(
        tg_uid=message.from_user.id, col=ChronogramUser.language
    )
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[[await get_default_close_button(user_lang)]]
    )
...
async def process_subscription_menu(
    callback_query: CallbackQuery, callback_data: SubscriptionMenuCallback, l10n: L10N
):
