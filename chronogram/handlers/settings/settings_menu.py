...
async def get_init_settings_msg(tg_uid, l10n: L10N) -> str:
    ...
    msg = l10n.data['/settings']['init'].format({'en': '🇬🇧', 'ru': '🇷🇺'}[l10n.lang],
                                                (datetime.utcnow() + timedelta(minutes=utc_diff))
                                                .strftime('%H:%M %d.%m.%Y'),
    ...
    if await db_req.get_user_attr(tg_uid=tg_uid, col=ChronogramUser.subscription):
        deadline = await db_req.get_user_attr(tg_uid=tg_uid, col=ChronogramUser.subscription_deadline)
        deadline += timedelta(minutes=utc_diff)
        msg += l10n.data['/settings']['subscription_expires'].format(deadline.strftime('%H:%M %d.%m.%Y'))
    else:
...
async def start_settings_menu(tg_uid, l10n: L10N) -> InlineKeyboardMarkup:
    ...
    keyboard[0] = [InlineKeyboardButton(text=sub_msg_text,
                                        callback_data=SettingsCallback(action=SettingsMenuActions.SUBSCRIPTION).pack())]
    keyboard[1] = [InlineKeyboardButton(text=l10n.data['/settings']['timezone'],
                                        callback_data=SettingsCallback(action=SettingsMenuActions.TIMEZONE).pack())]
    keyboard[2] = [InlineKeyboardButton(text=l10n.data['/settings']['language'],
                                        callback_data=SettingsCallback(action=SettingsMenuActions.LANGUAGE).pack())]
    keyboard[3] = [InlineKeyboardButton(text=l10n.data['/settings']['close'],
...
async def _select_language_menu(l10n: L10N) -> InlineKeyboardMarkup:
    ...
    builder.add(InlineKeyboardButton(text='🇬🇧 English', callback_data=SettingsCallback(action=SettingsMenuActions
                                                                                       .SELECT_LANGUAGE + 'en').pack()))
    builder.add(InlineKeyboardButton(text='🇷🇺 Русский', callback_data=SettingsCallback(action=SettingsMenuActions
                                                                                       .SELECT_LANGUAGE + 'ru').pack()))
    builder.add(InlineKeyboardButton(text=l10n.data['buttons']['back'],
                                     callback_data=SettingsCallback(action=SettingsMenuActions.BACK).pack()))
    builder.adjust(1)
    return builder.as_markup()


async def process_selection(callback: CallbackQuery, data: SettingsCallback, l10n: L10N):
    match data.action:
        case SettingsMenuActions.SUBSCRIPTION:
            await callback.message.edit_reply_markup(reply_markup=await choose_duration_menu(
                data=SubscriptionMenuCallback(user_id=callback.from_user.id), l10n=l10n))
        case SettingsMenuActions.TIMEZONE:
            user_utc_diff = await db_req.get_user_attr(tg_uid=callback.from_user.id,
                                                       col=ChronogramUser.utc_offset_minutes)
            hour = user_utc_diff // 60
            minute = user_utc_diff % 60
            data = UtcPickerCallback(tg_uid=callback.from_user.id, hour=f"{abs(hour):02d}", minute=f"{abs(minute):02d}",
    ...
    if data.action.startswith(SettingsMenuActions.SELECT_LANGUAGE):
        ...
        old_val = await db_req.get_user_attr(tg_uid=callback.from_user.id, col=ChronogramUser.utc_offset_minutes)
        if old_val == user_utf_diff_minutes:
            await callback.answer()
        else:
            await db_req.edit_utc_diff(tg_uid=callback.from_user.id, value=user_utf_diff_minutes)
            await callback.message.edit_text(await get_init_settings_msg(callback.from_user.id, l10n=l10n))
            await callback.message.edit_reply_markup(reply_markup=await start_utc_picker(
                data=UtcPickerCallback(tg_uid=callback.from_user.id, sign=sign, hour=hour, minute=minute), l10n=l10n))
