...
async def get_current_value(data: UtcPickerCallback):
    return (
        datetime.utcnow()
        + timedelta(
            hours=int(data.sign + str(data.hour)),
            minutes=int(data.sign + str(data.minute)),
        )
    ).strftime("%H:%M")


async def process_utc_picker(
    callback: CallbackQuery, data: UtcPickerCallback, l10n: L10N
):
    ...
    match data.action:
        ...
        case UtcPickerActions.increase_hour | UtcPickerActions.decrease_hour:
            data.hour = (UTC_HOUR_VALUES * 2)[
                UTC_HOUR_VALUES.index(data.hour)
                + {
                    UtcPickerActions.increase_hour: 1,
                    UtcPickerActions.decrease_hour: -1,
                }[data.action]
            ]
            await callback.message.edit_reply_markup(
                reply_markup=await start_utc_picker(data, l10n=l10n)
        ...
        case UtcPickerActions.increase_minute | UtcPickerActions.decrease_minute:
            data.minute = (UTC_MINUTE_VALUES * 2)[
                UTC_MINUTE_VALUES.index(data.minute)
                + {
                    UtcPickerActions.increase_minute: 1,
                    UtcPickerActions.decrease_minute: -1,
                }[data.action]
            ]
            await callback.message.edit_reply_markup(
                reply_markup=await start_utc_picker(data, l10n=l10n)
...
async def start_utc_picker(data: UtcPickerCallback, l10n: L10N) -> InlineKeyboardMarkup:
    ...
    keyboard[0] = [
        InlineKeyboardButton(
            text=l10n.data["/settings"]["utc_picker_display_value"].format(
                await get_current_value(data)
            ),
    ...
    keyboard[4] = [
        ...
        InlineKeyboardButton(
            ...
            callback_data=SettingsCallback(
                action=SettingsMenuActions.SELECT_UTC + f"{data.sign}|{data.hour}|"
                f"{data.minute}"
            ).pack(),
        ),
    ]
