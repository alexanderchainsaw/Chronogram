...
async def command_settings(message: Message, state: FSMContext, l10n: L10N):
    ...
    await message.answer(
        text=await get_init_settings_msg(message.from_user.id, l10n=l10n),
        reply_markup=await start_settings_menu(tg_uid=message.from_user.id, l10n=l10n),
    )
    await message.delete()


@settings_router.callback_query(UtcPickerCallback.filter())
async def process_utc_picker_callback(
    callback_query: CallbackQuery, callback_data: UtcPickerCallback, l10n: L10N
):
...
async def process_settings_callback(
    callback_query: CallbackQuery, callback_data: SettingsCallback, l10n: L10N
):
