...
async def get_app_stats(message: Message, state: FSMContext):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[[await get_default_close_button("en")]]
    )
    await message.answer(
        f"• Total users: {data[0]}\n"
...
async def admin_message(message: Message, state: FSMContext):
    ...
    if len(message.text.split(maxsplit=2)) != 3:
        await message.answer(
            "Usage:\n<code>/admin_message *user_tg_id* *message text*</code>",
            reply_markup=keyboard,
        )
        return
...
async def forceful_refund(message: Message, state: FSMContext):
    ...
    if len(message.text.split()) != 3:
        await message.answer(
            "Usage:\n<code>/forcerefund *user_tg_id* *transaction_id*</code>",
            reply_markup=keyboard,
        )
        return
    _, user_id, transaction_id = message.text.split()
    match await db_req.process_refund(
        tg_uid=int(user_id), telegram_payment_charge_id=transaction_id
    ):
        case "INVALID_PAY_ID":
        ...
        case "SUCCESS":
            await config.BOT.refund_star_payment(
                user_id=int(user_id), telegram_payment_charge_id=transaction_id
            )
...
async def gift_subscription(message: Message, state: FSMContext):
    ...
    if len(message.text.split()) != 3:
        await message.answer(
            "Usage:\n<code>/gift_sub *user_tg_id* *months*</code>",
            reply_markup=keyboard,
        )
        return
    ...
    await message.answer(
        f"Granted {months} month sub to {user_id}", reply_markup=keyboard
    )
    await message.delete()
